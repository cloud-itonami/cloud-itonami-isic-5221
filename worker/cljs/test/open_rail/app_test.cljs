(ns open-rail.app-test
  (:require [cljs.test :refer [deftest is testing use-fixtures]]
            [re-frame.core :as rf]
            [re-frame.db :as rf-db]
            [open-rail.app :as app]))
...
(deftest initialize-db-sets-defaults
  (testing ":initialize-db populates every fact the Svelte scaffold held"
    ...
    (is (= "etzhayyim-project-open-rail" @(rf/subscribe [:app/project])))
    (is (= "worker" @(rf/subscribe [:app/kind])))
    (is (= 1 @(rf/subscribe [:app/route-count])))
    (is (= ["open-rail.etzhayyim.com/*"]
           @(rf/subscribe [:app/routes])))
    (is (= 4 (count @(rf/subscribe [:app/vars]))))
    (is (false? @(rf/subscribe [:app/xrpc?])))
    (is (= "worker/cljs/src/open_rail/app.cljs" @(rf/subscribe [:app/relative-path])))))
