(ns open-rail.app
  "etzhayyim-open-rail appview — reagent + re-frame, view built from
  ...
  Three fields are updated, not simplified-away-copied, and each is
  called out here so the port stays honest about what actually changed:
  ...
  - `:app/xrpc?` is now false. This migration's wrangler.jsonc drops
    `main` (see that file's header comment): neither Worker source in
    this repo calls `env.ASSETS.fetch`, so the XRPC route this page used
    to advertise as enabled no longer deploys. The XRPC handler itself is
    preserved byte-for-byte at `../src/xrpc-mcp-router-proxy.ts` (moved,
    not deleted, from `svelte/src/routes/xrpc/[...path]/+server.ts`) —
    backend Worker/XRPC code is out of scope for a frontend migration.
  - `:app/route-count`, `:app/routes` and `:app/vars` are read from
    `worker/wrangler.jsonc` (the live source of truth for both) rather
    than copied from the Svelte scaffold's own `app` const, which held
    `routeCount: 0, routes: [], vars: []` — stale placeholder zeros that
    were already inconsistent with the deployed config (1 route pattern,
    4 vars) before this migration touched anything. Porting the
    placeholder zeros forward would make this status page misreport its
    own Worker; reading wrangler.jsonc directly keeps the page accurate
    without inventing new facts. `:app/title`, `:app/project`,
    `:app/name` and `:app/kind` are copied verbatim from the Svelte
    scaffold even though they read like generic scaffold defaults
    (\"Worker\" / \"worker\") — those are not contradicted by any other
    source in this repo, so they are ported as-is.
  ...
  components or ext-rules change) with:
    ...
    (spit \"public/index.html\"
          (page/->page {:title \"etzhayyim-project-open-rail\"
                         :lang \"ja\"
                         :description \"etzhayyim-open-rail — railway operations Worker appview (reagent + re-frame + jp-go-dds).\"
                         :css (slurp (io/resource \"jp_go_dds/dds.css\"))}
                        [:div {:id \"app\"} \"etzhayyim-project-open-rail loading…\"]
                        [:noscript \"etzhayyim-project-open-rail requires JavaScript.\"]
...
;; See the namespace docstring for which three are read from
;; wrangler.jsonc / updated for this migration rather than copied as-is.

(def default-db
  {:app/title "Worker"
   :app/project "etzhayyim-project-open-rail"
   :app/name "worker"
   :app/kind "worker"
   :app/route-count 1
   :app/routes ["open-rail.etzhayyim.com/*"]
   :app/vars ["AGENTGATEWAY_MCP_ROUTER_URL" "APP_FRAMEWORK" "APP_HANDLE" "PRIMARY_DID"]
   :app/xrpc? false
   :app/relative-path "worker/cljs/src/open_rail/app.cljs"})
